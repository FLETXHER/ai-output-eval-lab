...
CREATE TABLE human_review_correction_targets (
    id INTEGER PRIMARY KEY,
    original_human_review_id INTEGER NOT NULL REFERENCES human_reviews(id),
    evaluation_result_id INTEGER NOT NULL REFERENCES evaluation_results(id),
...
CREATE TRIGGER human_review_correction_targets_match_evaluation
...
CREATE TRIGGER human_review_correction_targets_no_update
...
CREATE TRIGGER human_review_correction_targets_no_delete
...
CREATE TRIGGER human_review_corrections_requires_authorized_target
BEFORE INSERT ON human_review_corrections
FOR EACH ROW
WHEN NOT EXISTS (
    SELECT 1
